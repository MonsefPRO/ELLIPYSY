...
import { Home, ChevronRight, CheckCircle2, ShieldCheck, Zap, Award, User, Mail, Phone, FolderOpen, AlignLeft, MapPin, Star, Tag } from 'lucide-react';
...
export default function Devis() {
  ...
  const form = useSecureForm({
    ...
    onSubmit: async (data) => {
      // Version de base : On valide techniquement l'envoi pour afficher le succès
      // Les données sont logguées en local pour tes tests
          ...
          <div className="max-w-4xl mx-auto py-20 text-center animate-in fade-in zoom-in duration-500">
            ...
            <div className="bg-white border-2 border-green-100 rounded-[3rem] p-10 shadow-2xl">
              <h1 className="text-3xl md:text-5xl font-black text-[#233B72] mb-4 uppercase tracking-tighter">
                {isFr ? "Votre demande a été envoyée !" : "Request sent successfully!"}
              </h1>
              <p className="text-gray-600 text-lg md:text-xl font-bold">
                {isFr
                  ? "Merci pour votre message. Notre équipe analyse votre projet et vous répondra sous 24h ouvrées."
                  : "Thank you for your message. Our team is analyzing your project and will respond within 24 business hours."}
              </p>
            </div>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-[420px_1fr] gap-8 lg:gap-12">
            ...
            <aside className="hidden lg:block sticky top-32 space-y-6">
              <div className="bg-gradient-to-br from-[#233B72] to-blue-900 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden">
                ...
                <h3 className="text-2xl font-black mb-8 uppercase tracking-tighter">Expertise Ellipsys</h3>
                <div className="space-y-6">
                   <div className="flex gap-4 items-start"><Zap className="text-orange-400" /> <div><strong>Réponse sous 24h</strong><br/><span className="text-xs text-blue-200">Traitement prioritaire</span></div></div>
                   <div className="flex gap-4 items-start"><Award className="text-orange-400" /> <div><strong>Certifié DGAC</strong><br/><span className="text-xs text-blue-200">Sécurité et conformité</span></div></div>
                </div>
              </div>
            </aside>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="relative">
                    ...
                    <select name="service" required onChange={form.handleChange} className="w-full pl-14 pr-5 py-5 bg-gray-50 border-2 border-gray-100 rounded-2xl outline-none focus:border-brand-orange-500 transition-all font-bold appearance-none cursor-pointer">
                      <option value="">{isFr ? "Service souhaité *" : "Desired service *"}</option>
