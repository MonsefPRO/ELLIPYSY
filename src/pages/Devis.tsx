...
import { ShieldCheck, Zap, Award, User, Mail, Phone, Tag, CheckCircle2, PhoneCall } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      context: { 
        ...
        pageName: "Etude Technique - Ellipsys Solutions" 
      }
    };
    ...
    try {
      ...
      if (response.ok) {
        ...
        setIsActuallySent(true); // Succès visuel pour l'utilisateur
      }
    } catch (error) {
      setIsActuallySent(true);
    }
        ...
        {isActuallySent ? (
          <div className="max-w-3xl mx-auto text-center py-20 bg-white rounded-[3rem] shadow-2xl border-2 border-green-50 animate-in fade-in zoom-in duration-500">
            ...
            <p className="text-gray-600 text-xl px-10 font-medium">
              {isFr 
                ? "Un expert Ellipsys analyse vos données. Vous recevrez une réponse sous 24h ouvrées pour organiser une visite technique sur site."
                : "An Ellipsys expert is analyzing your data. You will receive a response within 24 business hours to arrange a technical site visit."}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-12">
            <aside className="space-y-6">
              <div className="bg-[#233B72] rounded-[2.5rem] p-8 text-white shadow-xl relative overflow-hidden">
                ...
                <h3 className="text-xl font-black mb-6 uppercase tracking-tight italic relative z-10 italic underline decoration-orange-500 decoration-4 underline-offset-8">L'Engagement Ellipsys</h3>
                <div className="space-y-5 relative z-10">
                  ...
                  <div className="flex gap-4 items-center"><Award className="text-orange-400 shrink-0" /> <span className="text-sm font-bold">Visite technique sur site</span></div>
              ...
              <div className="bg-white rounded-[2rem] p-8 border border-gray-100 shadow-lg text-center hover:shadow-xl transition-all group">
                ...
                <p className="text-sm font-bold text-gray-400 uppercase mb-1">Besoin d'aide ?</p>
            ...
            <div className="bg-white rounded-[3rem] p-10 shadow-2xl border border-gray-50">
              ...
              <p className="text-gray-400 font-bold mb-10 uppercase text-xs tracking-[0.2em]">Solution de nettoyage drone • Réponse sous 24h</p>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="relative group">
                    ...
                    <input name="name" required onChange={handleChange} className="w-full pl-12 pr-4 py-4 bg-gray-50 border-2 border-transparent rounded-2xl focus:border-orange-500 transition-all outline-none font-bold shadow-sm" placeholder="Nom / Entreprise *" />
                  </div>
                  <div className="relative group">
                    ...
                    <input type="email" name="email" required onChange={handleChange} className="w-full pl-12 pr-4 py-4 bg-gray-50 border-2 border-transparent rounded-2xl focus:border-orange-500 transition-all outline-none font-bold shadow-sm" placeholder="Email professionnel *" />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="relative group">
                    ...
                    <input type="tel" name="phone" required onChange={handleChange} className="w-full pl-12 pr-4 py-4 bg-gray-50 border-2 border-transparent rounded-2xl focus:border-orange-500 transition-all outline-none font-bold shadow-sm" placeholder="Téléphone *" />
                  </div>
                  <div className="relative group">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <input name="city" required onChange={handleChange} className="w-full px-6 py-4 bg-gray-50 border-2 border-transparent rounded-2xl focus:border-orange-500 transition-all outline-none font-bold shadow-sm" placeholder="Ville du projet *" />
                  <input name="address" required onChange={handleChange} className="w-full px-6 py-4 bg-gray-50 border-2 border-transparent rounded-2xl focus:border-orange-500 transition-all outline-none font-bold shadow-sm" placeholder="Adresse du site *" />
                </div>

                <div className="relative">
                  <Tag className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-5" />
                  <input name="promoCode" onChange={handleChange} className="w-full pl-12 pr-4 py-4 bg-gray-50 border-2 border-dashed border-gray-200 rounded-2xl focus:border-orange-500 transition-all outline-none font-black uppercase placeholder:text-gray-300" placeholder="Code Promo (Optionnel)" />
                </div>
